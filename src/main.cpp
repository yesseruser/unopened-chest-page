...
class $modify(MyRewardsPage, SecretRewardsLayer) {
  void createSecondaryLayer(int p0) {
    ...
    for (int i = 0; i < page_count; i++) {
      const auto page =
          pages->getChildByType<CCLayer>(i)->getChildByType<CCMenu>(0);
      log::debug("onChestType page {} obtained", i);
      for (int j = 0; j < page->getChildrenCount(); j++) {
        const auto chest = page->getChildByType<CCMenuItemSpriteExtra>(j)
                               ->getChildByType<GJChestSprite>(0);
    ...
    log::debug("onChestType first unopened page obtained: {}",
               first_unopened_page);
